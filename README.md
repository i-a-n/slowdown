...
A JavaScript HTML to Markdown converter, forked from [Showdown](https://github.com/showdownjs/showdown) with critical bug fixes.

## What is Slowdown?

Slowdown is a focused fork of the Showdown library, specialized for **HTML → Markdown conversion only**. 

### Why fork Showdown?

The original Showdown library is no longer actively maintained and has several critical bugs in HTML-to-Markdown conversion:

1. **Tables without `<thead>` are not converted properly**
2. **HTML tags within `<pre>` tags cause `.toLowerCase()` errors**
3. **Consecutive `<pre>` tags are skipped due to whitespace node removal**

Slowdown fixes these issues while removing Markdown→HTML conversion code to focus solely on HTML→Markdown.

## License

Slowdown is released under the MIT license, maintaining compatibility with the original Showdown project.
